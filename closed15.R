...
closed15 <- function(X1, y1, X2, y2, constant=FALSE){ # fit1; first layer, fit2; last layer's emulator such as f_M(X2, f1(x2))
  if(constant){
...
closed215 <- function(X1, y1, X2, y2, X3, y3, constant=FALSE){ # fit1; first layer, fit2; second layer f_M(X2, f1(X2)), fit3; last layer f_H(X3, f_M(X3, f1(X3)))
  if(constant){
    closed1 <- closed15(X1, y1, X2, y2, constant=TRUE)
    
    fit1 <- closed1$fit1
    fit2 <- closed1$fit2
    fit3 <- matGP(cbind(X3, pred.matGP(fit2, cbind(X3, pred.matGP(fit1, X3)$mu))$mu), y3, nu=1.5, constant=TRUE)
  }else{
    closed1 <- closed15(X1, y1, X2, y2)
...
predclosed215 <- function(fit, x){
  ...
  if(constant){
    d <- ncol(fit1$X)
    x <- matrix(x, ncol=d)
    closed1 <- predclosed15(closed1, x)
    x.mu <- closed1$mu
    sig2 <- closed1$sig2 
...
# zetafun15 <- function(w1, w2, m, s){ # i < j -> w1.x2[k] < w1.x2[l]
#   # w1=w1.x2[k]; w2=w1.x2[l]; m=x.mu[i]; s=sig2[i];
#   if(w1 < w2){
#     muc <- m - 2*sqrt(3)*s/theta[d+1]
#     mud <- m + 2*sqrt(3)*s/theta[d+1]
#     
#     lambda31 <- c(1, muc, muc^2+s)
#     lambda32 <- c(0, 1, muc+w2)
#     lambda41 <- c(1, m, m^2+s)
...
#     lambda52 <- c(0, 1, -mud-w1)
#     
#     e3 <- c(1+(3*w1*w2 - sqrt(3)*theta[d+1]*(w1+w2))/theta[d+1]^2,
#             (2*sqrt(3)*theta[d+1] - 3*(w1+w2))/theta[d+1]^2,
#             3/theta[d+1]^2)
#     e4 <- c(1+(-3*w1*w2 + sqrt(3)*theta[d+1]*(w2-w1))/theta[d+1]^2,
#             3*(w1+w2)/theta[d+1]^2,
#             -3/theta[d+1]^2)
#     e5 <- c(1+(3*w1*w2 + sqrt(3)*theta[d+1]*(w1+w2))/theta[d+1]^2,
#             (2*sqrt(3)*theta[d+1] + 3*(w1+w2))/theta[d+1]^2,
#             3/theta[d+1]^2)
#     
#     
#     return(exp((6*s + sqrt(3)*theta[d+1]*(w1 + w2 - 2*m))/theta[d+1]^2) *
#              (e3 %*% lambda31 * pnorm((muc - w2)/sqrt(s)) +
#                 e3 %*% lambda32 * sqrt(s)/sqrt(2*pi) * exp(-(w2 - muc)^2/(2*s))) +
#              exp(-(sqrt(3)*(w2-w1)/theta[d+1])) *
#              (e4 %*% lambda41 * (pnorm((w2 - m)/sqrt(s)) - pnorm((w1 - m)/sqrt(s))) +
#                 e4 %*% lambda42 * sqrt(s)/sqrt(2*pi) * exp(-(w1 - m)^2/(2*s)) -
#                 e4 %*% lambda43 * sqrt(s)/sqrt(2*pi) * exp(-(w2 - m)^2/(2*s)) ) +
#              exp((6*s - sqrt(3)*theta[d+1]*(w1 + w2 - 2*m))/theta[d+1]^2) *
#              (e5 %*% lambda51 * pnorm((w1 - mud)/sqrt(s)) +
#                 e5 %*% lambda52 * sqrt(s)/sqrt(2*pi) * exp(-(w1 - mud)^2/(2*s))))
#   }else{ # b <= a
#     muc <- m - 2*sqrt(3)*s/theta[d+1]
...
#             (2*sqrt(3)*theta[d+1] - 3*(w1+w2))/theta[d+1]^2,
#             3/theta[d+1]^2)
#     e4 <- c(1+(-3*w1*w2 + sqrt(3)*theta[d+1]*(w1-w2))/theta[d+1]^2,
#             3*(w1+w2)/theta[d+1]^2,
#             -3/theta[d+1]^2)
#     e5 <- c(1+(3*w1*w2 + sqrt(3)*theta[d+1]*(w1+w2))/theta[d+1]^2,
#             (2*sqrt(3)*theta[d+1] + 3*(w1+w2))/theta[d+1]^2,
