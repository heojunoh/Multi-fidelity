...
### cost; 1, 2, 3 ###
which.max(alcfast/c(2,(2+4),(2+4+8)))
alcfast/c(2,(2+4),(2+4+8))


chosen <- matrix(0, ncol=2)
chosen[1,1] <- which.max(alcfast/c(2,(2+4),(2+4+8)))
...
### Add point ###
#################
Iselect <- IMSPE2select(g, g[which.min(Icand2fast)], fit.GP1, fit.GP2new, fit.GP3new, constant=TRUE) # This need to be changed according to the previous result.
...
chosen <- rbind(chosen, c(which.max(alcfast/c(2,(2+4),(2+4+8))), which.min(cbind(Icand1fast, Icand2fast, Icand3fast)[,which.max(alcfast/c(2,(2+4),(2+4+8)))])))
...
points(t(t(Iselect$fit1new$X)*attr(Iselect$fit1new$X,"scaled:scale")+attr(Iselect$fit1new$X,"scaled:center")), Iselect$fit1new$y, pch="1", col="red") # Change as Iselect$fit$X1, then the added new one can be reflected.
points(t(t(Iselect$fit2new$X)*attr(Iselect$fit2new$X,"scaled:scale")+attr(Iselect$fit2new$X,"scaled:center"))[,1], Iselect$fit2new$y, pch="2", col="red")
points(t(t(Iselect$fit3new$X)*attr(Iselect$fit3new$X,"scaled:scale")+attr(Iselect$fit3new$X,"scaled:center"))[,1], Iselect$fit3new$y, pch="3", col="red")
