...
for(kk in 1:10){
  ...
  intgvr <- integvar(matrix(x[which.max(predsig2),], nrow=1), fit.closed, mc.sample=5)
  ...
  while(nonlinear.cost[length(nonlinear.cost)] < 100){ # if total cost is less than the budget
    ...
    intgvr <- integvar(matrix(x[which.max(predsig2),], nrow=1), Iselect$fit, mc.sample=5)
    ...
    chosen <- rbind(chosen, c(which.max(alcfast/c(5,(5+10))), which.max(predsig2)))
