...
NestDesign$PX <- X1
NestDesign$ind[[1]] <- c(1:24)#[-c(11,12,15,16,19)]#c(1,2,3,4,5,6,8,9,10,12,13,14,15,16,17,18,20,21,22,24)
...
ytest <- c(25.82, 19.77, 20.52, 18.78, 24.68, 22.30, #4.55, 
           23.33, 32.85, 34.80, 36.11, 27.36)


### closed ###
tic.closed <- proc.time()[3]
fit.closed <- RNAmf(X1, y1, X2, y2, kernel="sqex", constant=TRUE)
predy <- predRNAmf(fit.closed, Xtest)$mu
predsig2 <- predRNAmf(fit.closed, Xtest)$sig2
...
### error ###
pred.qian <- c(23.57, 23.61, 20.20, 16.81, 26.01, 22.76, #10.52, 
               23.18, 37.07, 34.33, 35.90, 26.04)
pred.NARGP <- c(23.33626636,30.22667487,19.63168365,16.49967615,25.08493432,25.27920149, #7.31457021, 
                23.12312089,37.9909632 ,36.71574654,33.97894074,25.03210843)
sig2.NARGP <- c(0.84524085,1.97200697,0.04497958,0.02113747,0.85698853,0.75130919,#2.56025317,
                0.09605465,6.25126967,4.86949487,2.03493435,1.99819732)

### SRMSE with outlier ###
sqrt(sum(((predy-ytest)/ytest)^2)/12) # closed form 0.1904997
sqrt(sum(((pred.muficokm$mean-ytest)/ytest)^2)/12) # Cokm 0.1591857
sqrt(sum(((pred.NARGP-ytest)/ytest)^2)/12) # NARGP 0.2468533
sqrt(sum(((pred.qian-ytest)/ytest)^2)/12) # Qian 0.3873585

### SRMSE without outlier ###
sqrt(sum(((predy-ytest)/ytest)^2)/12) # closed form 0.1019598
sqrt(sum(((pred.muficokm$mean-ytest)/ytest)^2)/12) # Cokm 0.08772471
sqrt(sum(((pred.NARGP-ytest)/ytest)^2)/12) # NARGP 0.1737008
sqrt(sum(((pred.qian-ytest)/ytest)^2)/12) # Qian 0.08112996

# ### RMSE ###
# sqrt(mean((predy-ytest)^2)) # 2.856331, 2.733367
# sqrt(mean((pred.muficokm$mean-ytest)^2)) # 2.316786, 2.842415
