...
Currin2 <- function(xx, l){
  ...
  term2 <- exp(-1.4*x1)*cos(3.5*pi*x2)
...
rep <- 100
result.currin2.rmse <- matrix(NA, rep, 4)
result.currin2.meanscore <- matrix(NA, rep, 4)
result.currin2.medscore <- matrix(NA, rep, 4)
result.currin2.meancrps <- matrix(NA, rep, 4)
result.currin2.medcrps <- matrix(NA, rep, 4)
colnames(result.currin2.rmse) <- c("single", "closed", "direct", "KOH")
colnames(result.currin2.meanscore) <- c("single", "closed", "direct", "KOH") # The larger, the better
colnames(result.currin2.medscore) <- c("single", "closed", "direct", "KOH") # The larger, the better
colnames(result.currin2.meancrps) <- c("single", "closed", "direct", "KOH") # The smaller, the better
colnames(result.currin2.medcrps) <- c("single", "closed", "direct", "KOH") # The smaller, the better
...
for(i in 1:rep) {
  ...
  y3 <- apply(X3,1,Currin2, l=5)
  y2 <- apply(X2,1,Currin2, l=3)
  ...
  ### RMSE ###
  result.currin2.rmse[i,1] <- sqrt(mean((pred3$mu-apply(x,1,Currin2, l=Inf))^2)) # single fidelity
  result.currin2.rmse[i,2] <- sqrt(mean((predy-apply(x,1,Currin2, l=Inf))^2)) # closed form
  result.currin2.rmse[i,3] <- sqrt(mean((pred3new$mu-apply(x,1,Currin2, l=Inf))^2)) # not closed form
  # result.currin2.rmse[i,4] <- sqrt(mean((pred.muficokm$mean-apply(x,1,Currin2, l=Inf))^2)) # Cokriging
  result.currin2.rmse[i,4] <- sqrt(mean((mx2-apply(x,1,Currin2, l=Inf))^2)) # KOH
  
  result.currin2.meanscore[i,1] <- mean(score(apply(x,1,Currin2, l=Inf), pred3$mu, pred3$sig2)) # single fidelity
  result.currin2.meanscore[i,2] <- mean(score(apply(x,1,Currin2, l=Inf), predy, predsig2)) # closed form
  result.currin2.meanscore[i,3] <- mean(score(apply(x,1,Currin2, l=Inf), pred3new$mu, pred3new$sig2)) # not closed form
  # result.currin2.meanscore[i,4] <- mean(score(apply(x,1,Currin2, l=Inf), pred.muficokm$mean, pred.muficokm$sig2)) # Cokriging
  result.currin2.meanscore[i,4] <- mean(score(apply(x,1,Currin2, l=Inf), mx2, koh.var2)) # KOH
  
  result.currin2.medscore[i,1] <- median(score(apply(x,1,Currin2, l=Inf), pred3$mu, pred3$sig2)) # single fidelity
  result.currin2.medscore[i,2] <- median(score(apply(x,1,Currin2, l=Inf), predy, predsig2)) # closed form
  result.currin2.medscore[i,3] <- median(score(apply(x,1,Currin2, l=Inf), pred3new$mu, pred3new$sig2)) # not closed form
  # result.currin2.medscore[i,4] <- median(score(apply(x,1,Currin2, l=Inf), pred.muficokm$mean, pred.muficokm$sig2)) # Cokriging
  result.currin2.medscore[i,4] <- median(score(apply(x,1,Currin2, l=Inf), mx2, koh.var2)) # KOH
  
  result.currin2.meancrps[i,1] <- mean(crps(apply(x,1,Currin2, l=Inf), pred3$mu, pred3$sig2)) # single fidelity
  result.currin2.meancrps[i,2] <- mean(crps(apply(x,1,Currin2, l=Inf), predy, predsig2)) # closed form
  result.currin2.meancrps[i,3] <- mean(crps(apply(x,1,Currin2, l=Inf), pred3new$mu, pred3new$sig2)) # not closed form
  # result.currin2.meancrps[i,4] <- mean(crps(apply(x,1,Currin2, l=Inf), pred.muficokm$mean, pred.muficokm$sig2)) # Cokriging
  result.currin2.meancrps[i,4] <- mean(crps(apply(x,1,Currin2, l=Inf), mx2, koh.var2)) # KOH
  
  result.currin2.medcrps[i,1] <- median(crps(apply(x,1,Currin2, l=Inf), pred3$mu, pred3$sig2)) # single fidelity
  result.currin2.medcrps[i,2] <- median(crps(apply(x,1,Currin2, l=Inf), predy, predsig2)) # closed form
  result.currin2.medcrps[i,3] <- median(crps(apply(x,1,Currin2, l=Inf), pred3new$mu, pred3new$sig2)) # not closed form
  # result.currin2.medcrps[i,4] <- median(crps(apply(x,1,Currin2, l=Inf), pred.muficokm$mean, pred.muficokm$sig2)) # Cokriging
  result.currin2.medcrps[i,4] <- median(crps(apply(x,1,Currin2, l=Inf), mx2, koh.var2)) # KOH
  
}

par(mfrow=c(1,1))
#RMSE comparison#
apply(result.currin2.rmse, 2, mean)
table(apply(result.currin2.rmse, 1, which.min))
boxplot(result.currin2.rmse)

#score comparison, The larger, the better
apply(result.currin2.meanscore, 2, mean)
table(apply(result.currin2.meanscore, 1, which.max))
boxplot(result.currin2.meanscore)

#score comparison, The larger, the better
apply(result.currin2.medscore, 2, mean)
table(apply(result.currin2.medscore, 1, which.max))
boxplot(result.currin2.medscore)

#CRPS comparison, The smaller, the better
apply(result.currin2.meancrps, 2, mean)
table(apply(result.currin2.meancrps, 1, which.min))
boxplot(result.currin2.meancrps)

#CRPS comparison, The smaller, the better
apply(result.currin2.medcrps, 2, mean)
table(apply(result.currin2.medcrps, 1, which.min))
boxplot(result.currin2.medcrps)
...
closedrmse <- rbind(closedrmse, c(sort(result.currin2.rmse[,2])[5], apply(result.currin2.rmse, 2, mean)[2], sort(result.currin2.rmse[,2])[96]))
closedmeanscore <- rbind(closedmeanscore, c(sort(result.currin2.meanscore[,2])[5], apply(result.currin2.meanscore, 2, mean)[2], sort(result.currin2.meanscore[,2])[96]))
closedmedscore <- rbind(closedmedscore, c(sort(result.currin2.medscore[,2])[5], apply(result.currin2.medscore, 2, mean)[2], sort(result.currin2.medscore[,2])[96]))
closedmeancrps <- rbind(closedmeancrps, c(sort(result.currin2.meancrps[,2])[5], apply(result.currin2.meancrps, 2, mean)[2], sort(result.currin2.meancrps[,2])[96]))
closedmedcrps <- rbind(closedmedcrps, c(sort(result.currin2.medcrps[,2])[5], apply(result.currin2.medcrps, 2, mean)[2], sort(result.currin2.medcrps[,2])[96]))

kohrmse <- rbind(kohrmse, c(sort(result.currin2.rmse[,4])[5], apply(result.currin2.rmse, 2, mean)[4], sort(result.currin2.rmse[,4])[96]))
kohmeanscore <- rbind(kohmeanscore, c(sort(result.currin2.meanscore[,4])[5], apply(result.currin2.meanscore, 2, mean)[4], sort(result.currin2.meanscore[,4])[96]))
kohmedscore <- rbind(kohmedscore, c(sort(result.currin2.medscore[,4])[5], apply(result.currin2.medscore, 2, mean)[4], sort(result.currin2.medscore[,4])[96]))
kohmeancrps <- rbind(kohmeancrps, c(sort(result.currin2.meancrps[,4])[5], apply(result.currin2.meancrps, 2, mean)[4], sort(result.currin2.meancrps[,4])[96]))
kohmedcrps <- rbind(kohmedcrps, c(sort(result.currin2.medcrps[,4])[5], apply(result.currin2.medcrps, 2, mean)[4], sort(result.currin2.medcrps[,4])[96]))
