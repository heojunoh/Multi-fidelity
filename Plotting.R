...
rnacomptime <- c(result.nonlinear.comptime[,1],result.branin.comptime[,1],result.park.comptime[,1],result.borehole.comptime[,1],result.currin.comptime[,1],result.frank.comptime[,1])
cokmcomptime <- c(result.nonlinear.comptime[,2],result.branin.comptime[,2],result.park.comptime[,2],result.borehole.comptime[,2],result.currin.comptime[,2],result.frank.comptime[,2])
nargpcomptime <- c(c(12.638431072235107, 14.603119134902954, 9.038669109344482, 12.611816167831421, 8.181637287139893, 
...
borehole.comptime <- c(rnacomptime,cokmcomptime,nargpcomptime)

model <- c(rep("RNAmf",600),rep("CoKriging",600),rep("NARGP",600))
model <- factor(model, levels=c("RNAmf", "CoKriging", "NARGP"))


ggplot(data.frame(borehole.comptime, model), aes(x=model, y=borehole.comptime, fill=model)) + 
  geom_boxplot(alpha=0.5)  + 
  theme_ipsum() +  ggtitle("Computation time") +
  theme(axis.title.x = element_text(margin = margin(t = 10), hjust=0.5),
        # axis.text.x = element_blank(),
        axis.title.y = element_text(margin = margin(r = 10), hjust=0.5),
        text=element_text(size=16,  family="serif"),
...
# ggarrange(p1, p2, p3, p7, p8, p9, ncol=3, nrow=2, common.legend = TRUE, legend="bottom")

mean(nonlinear.l2error[1:100]) # 0.1194902
mean(nonlinear.l2error[101:200]) # 0.4606238
mean(nonlinear.l2error[201:300]) # 0.3986434


mean(branin.l2error[1:100]) # 31.06471
mean(branin.l2error[101:200]) # 28.43288
mean(branin.l2error[201:300]) # 64.55277


mean(park.l2error[1:100]) # 0.05314314
mean(park.l2error[101:200]) # 0.2178094
mean(park.l2error[201:300]) # 0.04281826


mean(nonlinear.meancrps[1:100]) # 0.05420367
mean(nonlinear.meancrps[101:200]) # 0.2705095
mean(nonlinear.meancrps[201:300]) # 0.2337974


mean(branin.meancrps[1:100]) # 16.06364
mean(branin.meancrps[101:200]) # 14.43492
mean(branin.meancrps[201:300]) # 38.22191


mean(park.meancrps[1:100]) # 0.02538141
mean(park.meancrps[101:200]) # 0.1071777
mean(park.meancrps[201:300]) # 0.02250967


mean(nonlinear.comptime[1:100]) # 0.11149
mean(nonlinear.comptime[101:200]) # 0.02974
mean(nonlinear.comptime[201:300]) # 5.873101


mean(branin.comptime[1:100]) # 0.39223
mean(branin.comptime[101:200]) # 0.05206
mean(branin.comptime[201:300]) # 38.777


mean(park.comptime[1:100]) # 0.57344
mean(park.comptime[101:200]) # 0.04703
mean(park.comptime[201:300]) # 34.18677
