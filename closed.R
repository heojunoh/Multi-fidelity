...
closed <- function(x, fit1, fit2, constant=FALSE){ # fit1; first layer, fit2; last layer's emulator such as f_M(X2, f1(x2))
  if(constant){
    c <- 0.001 * 1/attr(scale(fit1$y, center=FALSE), "scaled:scale")
    
    d <- ncol(fit1$X)
    x <- matrix(x, ncol=d)
    x.mu <- pred.GP(fit1, x)$mu # mean of f1(u)
    sig2 <- c^2*pred.GP(fit1, x)$sig2 # variance of f1(x)
    ...
    return(list(mu=1/c*predy, sig2=1/c^2*predsig2))
  }else{
    c <- 0.001 * 1/attr(scale(fit1$y, center=FALSE), "scaled:scale")
    ...
    Ci <- fit2$Ki
    a <- Ci %*% (c*(y2 + attr(y2, "scaled:center")))
...
closed2 <- function(x, fit1, fit2, fit3, constant=FALSE){ # fit1; first layer, fit2; second layer f_M(X2, f1(X2)), fit3; last layer f_H(X3, f_M(X3, f1(X3)))
  if(constant){
    c <- 0.001 * 1/attr(scale(fit2$y, center=FALSE), "scaled:scale")
    ...
    Ci <- fit3$Ki
    a <- Ci %*% (c*(y3 + attr(y3, "scaled:center")))
