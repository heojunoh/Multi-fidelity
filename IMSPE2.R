...
IMSPE2select <- function(x, newx, fit1, fit2, fit3, mc.sample=10000, constant=FALSE){ 
  ...
  y1.select <- fl(newx, l=1) ### need to change for different function
  y2.select <- fl(newx, l=3) ### need to change for different function
...
# IMSPE2fast1 <- function(x, newx, fit1, fit2, mc.sample=10000, constant=FALSE){ 
#   ### This is updating Ki using Ki_{n+1}'s closed form ###
#   # x; usually grid
#   # newx; new point which will be added, should be n=1
#   # fit1; first layer's emulator
...
#   
#   if(constant){
#     return(list(IMSPE=mean(closed(x, fit1, fit2, constant=TRUE)$sig2), fit1new=fit1, fit2new=fit2))
#   }else{
#     return(list(IMSPE=mean(closed(x, fit1, fit2)$sig2), fit1new=fit1, fit2new=fit2))
#   }
# }


IMSPE2select1 <- function(x, newx, fit1, fit2, mc.sample=10000, constant=FALSE){ 
  ### This is updating Ki using Ki_{n+1}'s closed form ###
  # x; usually grid
  # newx; new point which will be added, should be n=1
  # fit1; first layer's emulator
  ...
  y2.select <- f2(newx) ### need to change for different function
